...
func handleSignal() {
	...
	// 监听信号
	signal.Notify(ch, syscall.SIGINT, syscall.SIGTERM, syscall.SIGHUP, syscall.SIGQUIT, syscall.SIGKILL)
	for {
		sig := <-ch
		log.Printf("signal receive: %v\n", sig)
		ctx, _ := context.WithTimeout(context.Background(), 20*time.Second)
		switch sig {
		case syscall.SIGHUP, syscall.SIGTERM: // 终止进程执行
			log.Println("shutdown")
			signal.Stop(ch)
			server.Shutdown(ctx)
			log.Println("graceful shutdown")
			return
		case syscall.SIGINT: // 进程热重启
			log.Println("reload")
			err := reload() // 执行热重启函数
			if err != nil {
		...
		case syscall.SIGQUIT:
			log.Println("graceful SIGQUIT")
		case syscall.SIGKILL:
			log.Println("graceful SIGKILL")
			return
		}
	}
}
